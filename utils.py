...
def get_template_path(template_id, side="front"):
    ...
    try:
        ...
        if template:
            ...
            # Prefer local template file when available to avoid transient network failures.
...
def get_default_photo_config():
    ...
    return {
        ...
        "remove_background": True,       # ← ADD THIS LINE
...
def parse_layout_config(layout_config):
    ...
    if isinstance(objects, list):
        for idx, obj in enumerate(objects):
            ...
            if obj_type not in {"text", "rect", "circle", "line", "image"}:
                continue
            sanitized = {
...
def get_field_layout_item(layout_config_raw, field_key, default_label_x, default_value_x, default_y, text_direction="ltr"):
    """
    ...
    for part, prefix in (("label", "label"), ("value", "value"), ("colon", "colon")):
        ...
        if "y" in part_obj:
            try:
                ...
                pass
        if "visible" in part_obj:
            result[f"{prefix}_visible"] = bool(part_obj.get("visible"))
        if "grow" in part_obj:
...
def get_storage_backend():
    ...
    if mode in {"cloudinary", "cloud"} and not configured:
        logger.error("Cloudinary credentials are missing but STORAGE_BACKEND=cloudinary was requested; using local storage.")
        return "local"
