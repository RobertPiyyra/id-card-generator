...
from models import db, Student, Template, TemplateField, ActivityLog, NotificationPreference, NotificationLog, KeyboardLanguagePreference
...
try:
    ...
    print(f"⚠️ rembg disabled: {e}")

from PIL import Image, ImageOps

# ---- ADD HERE ----
Image.open_original = Image.open

def _is_student_upload_path(path_value):
    """
    Apply smart photo processing only for real student-upload paths.
    Avoid template paths like `templates_uploads`, which are not photos.
    """
    try:
        text = str(path_value or "").replace("\\", "/").lower()
    except Exception:
        return False
    if not text:
        return False
    if "/templates_uploads/" in text:
        return False
    return "/uploads/" in text or text.endswith("/uploads")

def smart_open(path, *args, **kwargs):
    img = Image.open_original(path, *args, **kwargs)

    if _is_student_upload_path(path):
        img = _process_photo_pil(img)

    return img

Image.open = smart_open
# ------------------
...
def apply_layout_custom_objects_pil(template_img, template_obj, font_settings, side="front", language="english"):
    ...
    for obj in objects:
        ...
        stroke_width = max(1, int(obj.get("stroke_width", 2) or 2))
        fill_rgba = tuple(fill) + (alpha,)
        stroke_rgba = tuple(stroke) + (alpha,)

        if obj_type == "rect":
            w = max(1, int(obj.get("width", 120) or 120))
            h = max(1, int(obj.get("height", 60) or 60))
            ...
            w = max(1, int(obj.get("width", 80) or 80))
            h = max(1, int(obj.get("height", w) or w))
            overlay = Image.new("RGBA", (w, h), (0, 0, 0, 0))
            ImageDraw.Draw(overlay).ellipse([0, 0, w, h], fill=fill_rgba, outline=stroke_rgba, width=stroke_width)
            _paste_rotated_overlay(overlay, x, y, angle)
        elif obj_type == "line":
            x2 = int(obj.get("x2", x + 120) or (x + 120))
            y2 = int(obj.get("y2", y) or y)
            min_x = min(x, x2)
            min_y = min(y, y2)
            ...
            try:
                ...
                w = max(1, int(obj.get("width", overlay.width) or overlay.width))
                h = max(1, int(obj.get("height", overlay.height) or overlay.height))
            ...
            font_size = max(8, int(obj.get("font_size", 24) or 24))
...
def render_student_card_side(template_obj, student_like, side="front", student_id=None, school_name=None):
    """Render one card side using side-aware template assets/settings."""
    if not template_obj:
        return None
    ...
    all_fields = [
        {'key': 'NAME', 'label': labels_map['NAME'], 'val': getattr(student_like, "name", "") or "", 'order': 10},
        {'key': 'F_NAME', 'label': labels_map['F_NAME'], 'val': getattr(student_like, "father_name", "") or "", 'order': 20},
        {'key': 'CLASS', 'label': labels_map['CLASS'], 'val': getattr(student_like, "class_name", "") or "", 'order': 30},
        {'key': 'DOB', 'label': labels_map['DOB'], 'val': getattr(student_like, "dob", "") or "", 'order': 40},
        {'key': 'MOBILE', 'label': labels_map['MOBILE'], 'val': getattr(student_like, "phone", "") or "", 'order': 50},
        {'key': 'ADDRESS', 'label': labels_map['ADDRESS'], 'val': getattr(student_like, "address", "") or "", 'order': 60},
    ]
    ...
    for item in all_fields:
        ...
        if layout_item["label_visible"]:
            ...
            draw_aligned_colon_pil(
                draw, card_width, direction, value_x_eff, colon_y_eff, colon_text_final, colon_font, colon_fill, lang, label_colon_gap,
                anchor_x=colon_x_eff, grow_mode=colon_grow_eff,
            )

        is_vertically_overlapping = (value_y_eff < p_bottom) and ((value_y_eff + line_height) > p_y)
        if direction == "rtl":
            right_anchor = card_width - value_x_eff
            max_w = right_anchor - 20
            if is_vertically_overlapping:
                p_right = p_x + p_w
                if 20 < p_right < right_anchor:
                    max_w = right_anchor - p_right - 15
        else:
            if is_vertically_overlapping and (p_x > value_x_eff):
                max_w = p_x - value_x_eff - 15
            else:
                max_w = card_width - value_x_eff - 20
        ...
        text_len = draw.textlength(display_val, font=value_font, **get_draw_text_kwargs(display_val, lang))
        if text_len > max_w:
            avg_char_w = value_font_size_eff * 0.55
            chars_limit = max(5, int(max_w / max(avg_char_w, 1)))
            wrapped = textwrap.wrap(raw_val, width=chars_limit, break_long_words=True)
            longest_display = process_text_for_drawing(max(wrapped, key=len), lang)
            value_font = load_font_dynamic(font_reg_path, longest_display, max_w, value_font_size_eff, language=lang)
            for line in wrapped:
                line_display = process_text_for_drawing(line, lang)
                if layout_item["value_visible"]:
                    value_draw_x = flip_x_for_text_direction(
                        value_x_eff, line_display, value_font, card_width, direction, draw=draw, grow_mode=layout_item["value_grow"]
                    )
                    draw.text((value_draw_x, value_y_eff), line_display, font=value_font, fill=value_fill, **get_draw_text_kwargs(line_display, lang))
                value_y_eff += line_height
    ...
    try:
        photo_img = load_student_photo_rgba(student_like, photo_settings["photo_width"], photo_settings["photo_height"], timeout=8)
        if photo_img:
            radii = [
        ...
        logger.error(f"Error adding {side} photo: {e}")
    ...
    photo_ref = getattr(student_like, "photo_url", None) or getattr(student_like, "photo_filename", None) or ""
    qr_id = generate_data_hash(form_data, photo_ref)[:10]
    ...
    if qr_settings.get("enable_qr", False):
        if qr_settings.get("qr_data_type") == "url":
            ...
            qr_payload = json.dumps({
                "student_id": student_id or qr_id,
                "name": getattr(student_like, "name", "") or "",
        ...
        qr_img = generate_qr_code(qr_payload, qr_settings, qr_size).resize((qr_size, qr_size))
    ...
    if qr_settings.get("enable_barcode", False):
        ...
        template_img.paste(barcode_img, (qr_settings.get("barcode_x", 50), qr_settings.get("barcode_y", 200)))

    apply_layout_custom_objects_pil(template_img, template_obj, font_settings, side=side, language=lang)
    return template_img
...
def load_student_photo_rgba(student, width, height, timeout=10):
    """Load the student's source photo as RGBA, from either a URL or local upload."""
    photo_url, local_path = resolve_student_photo_reference(student)

    if photo_url:
        response = requests.get(photo_url, timeout=timeout)
        response.raise_for_status()
        photo_img = Image.open(io.BytesIO(response.content)).convert("RGBA")
    elif local_path and os.path.exists(local_path):
        with open(local_path, "rb") as fh:
            photo_img = Image.open(fh).convert("RGBA")
    else:
        return None

    return photo_img.resize((width, height), Image.LANCZOS)
...
def resolve_student_card_preview_urls(student):
    ...
    if getattr(student, "image_url", None):
        ...
        preview_filename = str(student.generated_filename).replace(".pdf", ".jpg")
...
def auto_crop_face_photo(photo_path, target_width=260, target_height=313, remove_background=False, bg_color="#ffffff"):
    ...
    try:
        ...
        # If MediaPipe isn't available, don't error-spam; just do a safe center crop.
        ...
        img_np = np.array(detect_img)
        h_detect, w_detect = img_np.shape[:2]
...
def _process_photo_pil(pil_img, target_width=260, target_height=313, remove_background=False, bg_color="#ffffff"):
    ...
    try:
        ...
        # Respect EXIF orientation if present (mobile photos).
        try:
            pil_img = ImageOps.exif_transpose(pil_img)
        except Exception:
            pass
    ...
    if request.method == "POST":
        try:
            ...
            if 'photo' in request.files and request.files['photo'].filename:
                ...
                # Auto-crop using MediaPipe (when available) + optional background removal.
                try:
                ...
                if STORAGE_BACKEND == "local":
                    try:
                        ...
                        with open(local_photo_path, "wb") as fh:
                            fh.write(photo_bytes.getvalue())
                        photo_url = None
                    except Exception as e:
            ...
            for item in all_fields:
                ...
                # --- STANDARD LOGIC FOR OTHER FIELDS ---
                    ...
                    test_font = load_font_dynamic(FONT_REG, display_val, 10**9, value_font_size_eff, language=lang)
                    text_len = draw.textlength(display_val, font=test_font, **get_draw_text_kwargs(display_val, lang))
                    ...
                    if text_len > max_w:
                        ...
                        longest_display = process_text_for_drawing(max(wrapped, key=len), lang)
                        v_font = load_font_dynamic(FONT_REG, longest_display, max_w, value_font_size_eff, language=lang)
                        ...
                        v_font = load_font_dynamic(FONT_REG, display_val, max_w, value_font_size_eff, language=lang)
                        if layout_item["value_visible"]:
                            value_draw_x = flip_x_for_text_direction(
                                value_x_eff,
                                display_val,
            ...
            try:
                if photo_url:
                    # Download photo from Cloudinary. We prefer a face-cropped URL (if possible),
                    # but always fall back to the original URL so preview never "loses" the photo.
                    import requests

                    req_w = photo_settings.get("photo_width", 260)
                    req_h = photo_settings.get("photo_height", 313)
                    processed_url = get_cloudinary_face_crop_url(photo_url, req_w, req_h) or photo_url

                    resp = requests.get(processed_url, timeout=10)
                    if resp.status_code != 200 and processed_url != photo_url:
            ...
            back_template_img = render_student_card_side(
                template,
                side_render_student,
                side="back",
                school_name=school_name,
        ...
        try:
            ...
            for item in all_fields:
                ...
                # --- STANDARD FIELDS ---
                    ...
                    test_font = load_font_dynamic(FONT_REGULAR_PATH, display_val, 10**9, value_font_size_eff, language=lang)
                    text_len = draw.textlength(display_val, font=test_font, **get_draw_text_kwargs(display_val, lang))
                    ...
                    if text_len > max_w:
                        ...
                        longest_display = process_text_for_drawing(max(wrapped, key=len), lang)
                        v_font = load_font_dynamic(FONT_REGULAR_PATH, longest_display, max_w, value_font_size_eff, language=lang)
                        ...
                        v_font = load_font_dynamic(FONT_REGULAR_PATH, display_val, max_w, value_font_size_eff, language=lang)
                        if layout_item["value_visible"]:
                            value_draw_x = flip_x_for_text_direction(
                                value_x_eff,
                                display_val,
            ...
            try:
                ...
                if photo_url:
                    processed_url = get_cloudinary_face_crop_url(
                        photo_url,
                        photo_settings.get("photo_width", 260),
                        photo_settings.get("photo_height", 313),
                    ) or photo_url
                    resp = requests.get(processed_url, timeout=10)
                    if resp.status_code != 200 and processed_url != photo_url:
                        resp = requests.get(photo_url, timeout=10)
                ...
                elif photo_stored and photo_stored != "placeholder.jpg":
                    local_photo_path = os.path.join(UPLOAD_FOLDER, photo_stored)
                    if os.path.exists(local_photo_path):
                        photo_img = Image.open(local_photo_path).convert("RGBA")
            ...
            back_jpg_name = f"card_{template_id}_{student_id}_{timestamp}_back.jpg" if back_template_img is not None else None
            pdf_name = f"card_{template_id}_{student_id}_{timestamp}.pdf"

            template = force_rgb(template)
            if back_template_img is not None:
                back_template_img = force_rgb(back_template_img)
            ...
            if STORAGE_BACKEND == "local":
                ...
                if back_jpg_name and back_template_img is not None:
                    with open(os.path.join(GENERATED_FOLDER, back_jpg_name), "wb") as fh:
                        buf = io.BytesIO()
                        back_template_img.save(buf, "JPEG", quality=95)
                        buf.seek(0)
                        fh.write(buf.getvalue())
                ...
                with open(os.path.join(GENERATED_FOLDER, pdf_name), "wb") as fh:
                    buf = io.BytesIO()
                    if back_template_img is not None:
                        template.save(buf, "PDF", save_all=True, append_images=[back_template_img], resolution=300)
                    else:
    ...
    try:
        ...
        for item in all_fields:
            ...
            # --- STANDARD FIELDS ---
                ...
                text_len = draw.textlength(display_val, font=v_font, **get_draw_text_kwargs(display_val, lang))

                if text_len > max_w:
                    avg_char_w = value_font_size_eff * 0.55
                    chars_limit = max(5, int(max_w / max(avg_char_w, 1)))
                    wrapped = safe_wrap_preview(raw_val, chars_limit)
...
def background_bulk_generate(task_id, template_id, excel_path, photo_map):
    ...
    with app.app_context():
        try:
            ...
            lang = (getattr(template_obj, 'language', 'english') or 'english').lower()
            direction = (getattr(template_obj, 'text_direction', 'ltr') or 'ltr').lower()

            # Font paths (language-aware fallback happens in load_font_dynamic)
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    ...
                    # 1. Match by student name using forgiving aliases.
                    for alias in photo_match_aliases(clean_name):
                        if alias in photo_map:
                            used_photo = photo_map[alias]
                            break
                    else:
                        # 2. Match by explicit photo reference columns in Excel.
                    ...
                    for item in all_fields:
                        ...
                        # Get Raw Values
                        raw_label = apply_text_case(item['label'], text_case)
                        raw_val = apply_text_case(item['value'], text_case)
                        ...
                        if item.get('key') == 'ADDRESS':
                            ...
                            while curr_size >= min_size:
                                # Load font at this size
                                v_font = load_font_dynamic(FONT_REGULAR, "X", 10**9, curr_size, language=lang)
                                ...
                                for word in words:
                                    test_line = current_line + (" " if current_line else "") + word
                                    measure_line = process_text_for_drawing(test_line, lang)
                                    if draw.textlength(measure_line, font=v_font, **get_draw_text_kwargs(measure_line, lang)) <= max_w:
                                        current_line = test_line
                                    else:
                                        if current_line:
                                            lines.append(current_line)
                            ...
                            for line in wrapped_addr[:2]:
                                line_display = process_text_for_drawing(line, lang)
                                spacing = max(curr_size + 4, line_height - 6)
                                if layout_item["value_visible"]:
                                    value_draw_x = flip_x_for_text_direction(
                                        value_x_eff,
                                        line_display,
                        ...
                        # --- STANDARD FIELDS ---
                            ...
                            temp_font = load_font_dynamic(FONT_REGULAR, display_val, 10**9, value_font_size_eff, language=lang)
                            ...
                            if text_len > max_w:
                                ...
                                longest_display = process_text_for_drawing(max(wrapped, key=len), lang)
                                v_font = load_font_dynamic(FONT_REGULAR, longest_display, max_w, value_font_size_eff, language=lang)
                                ...
                                v_font = load_font_dynamic(FONT_REGULAR, display_val, max_w, value_font_size_eff, language=lang)
                                if layout_item["value_visible"]:
                                    value_draw_x = flip_x_for_text_direction(
                                        value_x_eff,
                                        display_val,
                    ...
                    # Generate QR / Barcode (independent)
                    qr_id = data_hash[:10]
                    if qr_settings.get("enable_qr", False):
                    ...
                    apply_layout_custom_objects_pil(template_img, template_obj, font_settings, side="front", language=lang)
                    ...
                    if back_template_path:
                        try:
                            back_img = load_template_smart(back_template_path).resize((card_width, card_height))
                            if back_img.mode == 'RGBA':
                                back_bg = Image.new("RGB", back_img.size, (255, 255, 255))
                                back_bg.paste(back_img, mask=back_img.split()[3])
                                back_save_img = back_bg
                            else:
                                back_save_img = back_img.convert("RGB")
                        except Exception as e:
                            logger.warning(f"Bulk back template render failed for template {template_id}: {e}")
                    # -------------------------------------------------

                    ts = datetime.now().strftime("%Y%m%d%H%M%S%f")
                    base = f"card_{template_id}_{ts}_{idx}"
                    jpg_name = f"{base}.jpg"
                    pdf_name = f"{base}.pdf"
                    back_jpg_name = f"{base}_back.jpg" if back_save_img is not None else None
                    ...
                    if STORAGE_BACKEND == "local":
                        ...
                        with open(os.path.join(GENERATED_FOLDER, jpg_name), "wb") as fh:
                            ...
                            jpg_buffer.seek(0)
                            fh.write(jpg_buffer.getvalue())
                        if back_jpg_name and back_save_img is not None:
                            with open(os.path.join(GENERATED_FOLDER, back_jpg_name), "wb") as fh:
                                back_jpg_buffer = io.BytesIO()
                                back_save_img.save(back_jpg_buffer, "JPEG", quality=95)
                                back_jpg_buffer.seek(0)
                                fh.write(back_jpg_buffer.getvalue())

                        # Save PDF locally (legacy per-student PDF; preview JPG is derived from this name)
                        with open(os.path.join(GENERATED_FOLDER, pdf_name), "wb") as fh:
                            pdf_buffer = io.BytesIO()
                            if back_save_img is not None:
                                save_img.save(pdf_buffer, "PDF", resolution=300, save_all=True, append_images=[back_save_img])
                            else:
                                save_img.save(pdf_buffer, "PDF", resolution=300)
                            pdf_buffer.seek(0)
                            fh.write(pdf_buffer.getvalue())
                    else:
                    ...
                    student = Student(
                        ...
                        # Cloudinary URLs when deployed; local files when running locally.
                        image_url=None if STORAGE_BACKEND == "local" else jpg_url,
                        back_image_url=None if STORAGE_BACKEND == "local" else back_jpg_url,
                        pdf_url=None if STORAGE_BACKEND == "local" else pdf_url,
                        generated_filename=pdf_name if STORAGE_BACKEND == "local" else None,
                        back_generated_filename=back_jpg_name if STORAGE_BACKEND == "local" else None,
                        created_at=datetime.now(timezone.utc),
